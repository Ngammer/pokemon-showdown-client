...
export class Pokemon implements PokemonDetails, PokemonHealth {
	...
	copyVolatileFrom(pokemon: Pokemon, copySource: 'batonpass' | 'shedtail' | 'illusion' | 'doubleteam') {
...
export class Side {
	...
	addSideCondition(effect: Dex.Effect, persist: boolean) {
		...
		case 'mist':
			...
			break;
		case 'tailwind':
			this.sideConditions[condition] = [effect.name, 1, this.battle.gen >= 5 ? persist ? 6 : 4 : persist ? 6 : 4, 0];
			break;
		case 'luckychant':
			this.sideConditions[condition] = [effect.name, 1, 5, 0];
			break;
	...
	switchIn(pokemon: Pokemon, kwArgs: KWArgs, slot = pokemon.slot) {
		...
		if (['batonpass', 'zbatonpass', 'shedtail', 'doubleteam'].includes(effect.id)) {
			pokemon.copyVolatileFrom(this.lastPokemon!, effect.id === 'shedtail' ? 'shedtail' : effect.id === 'doubleteam' ? 'doubleteam' : 'batonpass');
	...
	switchOut(pokemon: Pokemon, kwArgs: KWArgs, slot = pokemon.slot) {
		const effect = Dex.getEffect(kwArgs.from);
		if (!['batonpass', 'zbatonpass', 'doubleteam'].includes(effect.id) &&
...
export class Battle {
	...
	runMinor(args: Args, kwArgs: KWArgs, nextArgs?: Args, nextKwargs?: KWArgs) {
		...
		case '-sidestart': {
			...
			let effect = Dex.getEffect(args[2]);
			side.addSideCondition(effect, !!kwArgs.utilityumbrella);
