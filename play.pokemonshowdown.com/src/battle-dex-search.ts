...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected formatType: 'doubles' | 'bdsp' | 'bdspdoubles' | 'rs' | 'frlg' | 'bw1' | 'letsgo' | 'metronome' | 'natdex' |
		...
		'svdlc1natdex' | 'stadium' | 'lc' | 'legendsza' | 'bug' | 'champions' | null = null;
	isDoubles = false;
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		const customBanlists = [
			...
			'tiershift', 'linked', '4v4doublesuu', 'pokebilitiesaaa', 'pokelike',
		];
		if (dex.gen >= 6) {
...
class BattleItemSearch extends BattleTypedSearch<'item'> {
	...
	override defaultFilter(results: SearchRow[]) {
		if (this.species && !this.dex.species.get(this.species).nfe) {
			...
			results.splice(results.findIndex(row => row[1] === 'ovalstone'), 1);
			return results;
		}
		return results;
	}
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	getBaseResults() {
		...
		if (this.formatType?.startsWith('bug')) lsetTable = lsetTable['gen9bugdex'];
		while (learnsetid) {
			...
			if (learnset) {
				for (let moveid in learnset) {
					...
					if (
						this.formatType?.startsWith('bug') &&
						BattleTeambuilderTable['gen9bugdex']?.nonstandardMoves.includes(moveid)
					) {
						continue;
					}
					if (
