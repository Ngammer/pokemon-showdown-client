	...
	load: function (bgUrl, bgid, hues) {
		...
		if (!bgid) {
			if (location.host === 'smogtours.psim.us') {
				...
				bgid = ['horizon', 'ocean', 'waterfall', 'shaymin', 'charizards', 'psday', 'nuevometa'][Math.floor(Math.random() * 6)];
			} else {
				$(document.body).css({
					background: '',
					'background-size': ''
		...
		if (!hues) {
			...
			case 'nuevometa':
				hues = ["37.159090909090914,74.57627118644066%", "10.874999999999998,70.79646017699115%", "179.51612903225808,52.10084033613446%", "20.833333333333336,36.73469387755102%", "192.3076923076923,80.41237113402063%", "210,29.629629629629633%"];
				attrib = '<a href="https://lit.link/en/seiryuuden" target="_blank" class="subtle">"Nuevo Meta" <small>background by Juansssss</small></a>';
				break;
			case 'psday':
