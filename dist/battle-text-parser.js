...
class BattleTextParser {
    ...
    gen = 9;
    turn = 0;
    curLineSection = 'break';
    lowercaseRegExp = undefined;
    ...
    pokemonName = (pokemon) => {
        ...
        if (pokemon.charAt(3) === ':')
            return BattleTextParser.escapeReplace(pokemon.slice(4).trim());
        else if (pokemon.charAt(2) === ':')
            return BattleTextParser.escapeReplace(pokemon.slice(3).trim());
        return `???pokemon:${pokemon}???`;
    };
    ...
    pokemon(pokemon) {
        ...
        const isNear = side === this.perspective || side === BattleTextParser.allyID(side);
    ...
    template(type, ...namespaces) {
        for (const namespace of namespaces) {
            ...
            if (BattleText[id] && type in BattleText[id]) {
                ...
                if (!BattleText[id][type])
                    return '';
                return BattleText[id][type] + '\n';
            }
        }
        if (!BattleText.default[type])
            return '';
    ...
    parseArgsInner(args, kwArgs) {
        ...
        switch (cmd) {
            ...
            case '-end': {
                ...
                if (!template)
                    template = this.template(templateId, effect);
            ...
            case '-ability': {
                ...
                if (oldAbility)
                    line1 += this.ability(oldAbility, pokemon);
            ...
            case '-endability': {
                ...
                if (ability)
                    return this.ability(ability, pokemon);
                const line1 = this.maybeAbility(kwArgs.from, kwArgs.of || pokemon);
            ...
            case '-enditem': {
                ...
                if (!template)
                    template = this.template('activateItem').replace('[ITEM]', this.effect(item));
            ...
            case '-singlemove': {
                ...
                if (!template)
                    template = this.template('start').replace('[EFFECT]', this.effect(effect));
            ...
            case '-weather': {
                ...
                if (!weather || weather === 'none') {
                    ...
                    if (!template)
                        return this.template('endFieldEffect').replace('[EFFECT]', this.effect(weather));
                    return template;
                }
                if (kwArgs.upkeep) {
