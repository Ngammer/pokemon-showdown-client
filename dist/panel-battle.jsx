...
class BattlesPanel extends PSRoomPanel {
    refresh = () => {
        ...
        return <PSPanelWrapper room={room} scrollable><div class="pad">
            ...
            <form class="search">
                ...
                : !room.battles.length ?
                    ...
                    :
                        room.battles.map(battle => this.renderBattleLink(battle))}</div>
			</div>
		</div></PSPanelWrapper>;
    }
}
    ...
    handleMessage(line) {
        ...
        switch (cmd) {
            ...
            case 'choose': {
                ...
                if (this.choices.isDone())
                    this.send(`/choose ${this.choices.toString()}`, true);
                this.update(null);
                return true;
            }
        }
...
function PokemonButton(props) {
    ...
    if (!pokemon) {
        return <button name="cmd" value={props.cmd} class={`${props.disabled ? 'disabled ' : ''}has-tooltip`} style={{ opacity: props.disabled === 'fade' ? 0.5 : 1 }} data-tooltip={props.tooltip}>
			(empty slot)
		</button>;
    }
    let hpColorClass;
...
class BattlePanel extends PSRoomPanel {
    send = (text) => {
        ...
        return [
            battle.farSide.active.map((pokemon, i) => {
                ...
                if (moveTarget === 'adjacentAlly' || moveTarget === 'adjacentAllyOrSelf') {
                    ...
                    if (Math.abs(userSlotCross - i) > 1)
                        disabled = true;
                }
                if (pokemon?.fainted)
                    pokemon = null;
                return <PokemonButton pokemon={pokemon} cmd={disabled ? `` : `/${moveChoice} +${i + 1}`} disabled={disabled && 'fade'} tooltip={`activepokemon|1|${i}`}/>;
            }).reverse(),
            ...
            battle.nearSide.active.map((pokemon, i) => {
                ...
                if (moveTarget === 'adjacentFoe') {
                    ...
                    if (Math.abs(userSlot - i) > 1)
                        disabled = true;
                }
                if (moveTarget !== 'adjacentAllyOrSelf' && userSlot === i)
                    disabled = true;
                if (pokemon?.fainted)
                    pokemon = null;
                return <PokemonButton pokemon={pokemon} cmd={disabled ? `` : `/${moveChoice} -${i + 1}`} disabled={disabled && 'fade'} tooltip={`activepokemon|0|${i}`}/>;
            }),
        ];
    }
    renderSwitchControls(request, choices) {
        ...
        return request.side.pokemon.map((serverPokemon, i) => {
            ...
            return <PokemonButton pokemon={serverPokemon} cmd={`/switch ${i + 1}`} disabled={cantSwitch} tooltip={`switchpokemon|${i}`}/>;
        });
    }
    renderTeamControls(request, choices) {
        return request.side.pokemon.map((serverPokemon, i) => {
            const cantSwitch = choices.alreadySwitchingIn.includes(i + 1);
            return <PokemonButton pokemon={serverPokemon} cmd={`/switch ${i + 1}`} noHPBar disabled={cantSwitch && 'fade'} tooltip={`switchpokemon|${i}`}/>;
        });
    }
    renderTeamList() {
        ...
        return <div class="switchcontrols">
			...
			<div class="switchmenu">
                ...
                return <PokemonButton pokemon={serverPokemon} cmd={``} noHPBar disabled={true} tooltip={`switchpokemon|${i}`}/>;
            })}
			</div>
		</div>;
    }
    renderChosenTeam(request, choices) {
        return choices.alreadySwitchingIn.map(slot => {
            ...
            return <PokemonButton pokemon={serverPokemon} cmd={`/switch ${slot}`} disabled tooltip={`switchpokemon|${slot - 1}`}/>;
        });
    }
    renderOldChoices(request, choices) {
        ...
        for (let i = 0; i < choices.choices.length; i++) {
            ...
            if (choice.choiceType === 'move') {
                ...
                if (choice.mega)
                    buf.push(`Mega Evolve and `);
                if (choice.ultra)
                    buf.push(`Ultra Burst and `);
                if (choice.max && active?.canDynamax)
                    buf.push(active?.canGigantamax ? `Gigantamax and ` : `Dynamax and `);
    ...
    renderPlayerControls() {
        ...
        if (!choices)
            return 'Error: Missing BattleChoiceBuilder';
