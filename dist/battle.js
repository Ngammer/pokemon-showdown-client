...
const battle_scene_stub_1 = require("./battle-scene-stub");
const battle_log_1 = require("./battle-log");
const battle_animations_1 = require("./battle-animations");
class Pokemon {
    name = '';
    speciesForme = '';
    /**
...
class Side {
    ...
    addPokemon(name, ident, details, replaceSlot = -1) {
        ...
        poke.reset();
        if (oldPokemon?.moveTrack.length)
    ...
    */
    ...
    getSwitchedPokemon(pokemonid, details) {
        ...
        for (let i = 0; i < side.pokemon.length; i++) {
            ...
            if (pokemon === side.lastPokemon && !side.active[slot])
                continue;
    ...
    runMajor(args, kwArgs, preempt) {
        switch (args[0]) {
            ...
            case 'badge': {
                ...
                // (don't allow duping)
                if (!side.badges.includes(badge))
                    side.badges.push(badge);
                this.scene.updateSidebar(side);
                break;
            }
            case 'teamsize': {
