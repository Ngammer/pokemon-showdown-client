...
class PSHeader extends preact.Component {
    handleDragEnter = (e) => {
        console.log('dragenter ' + e.dataTransfer.dropEffect);
        e.preventDefault();
        if (!PS.dragging)
            return; // TODO: handle dragging other things onto roomtabs
        ...
        if (draggedOverRoom === null)
            return; // should never happen
        if (draggingRoom === draggedOverRoom)
            return;
    ...
    handleDragStart = (e) => {
        const roomid = PS.router.extractRoomID(e.currentTarget.href);
        if (!roomid)
            return; // should never happen
        PS.dragging = { type: 'room', roomid };
    };
    dragOnto(fromRoom, toRoomList, toIndex) {
        // one day you will be able to rearrange mainmenu and rooms, but not today
        if (fromRoom === '' || fromRoom === 'rooms')
            return;
        if (fromRoom === PS[toRoomList][toIndex])
            return;
        if (fromRoom === '' && toRoomList === 'miniRoomList')
            return;
        const roomLists = ['leftRoomList', 'rightRoomList', 'miniRoomList'];
        let fromRoomList;
        let fromIndex = -1;
        ...
        if (toRoomList === 'leftRoomList' && toIndex === 0)
            toIndex = 1; // Home is always leftmost
        if (toRoomList === 'rightRoomList' && toIndex === PS.rightRoomList.length - 1)
            toIndex--; // Rooms is always rightmost
    ...
    renderRoomTab(id) {
        ...
        switch (room.type) {
            ...
            case 'battle':
                ...
                if (idChunks.length <= 1) {
                    if (idChunks[0] === 'uploadedreplay')
                        formatid = 'Uploaded Replay';
                }
                else {
                    formatid = idChunks[0];
                }
        ...
        return <li>
			<a class={className} href={`/${id}`} draggable={true} onDragEnter={this.handleDragEnter} onDragStart={this.handleDragStart}>
    ...
    render() {
        return <div id="header" class="header" style={this.props.style}>
			<img class="logo" src={`https://${Config.routes.client}/pokemonshowdownbeta.png`} srcset={`https://${Config.routes.client}/pokemonshowdownbeta@2x.png 2x`} alt="Pokémon Showdown! (beta)" width="146" height="44"/>
...
class UserRoom extends PSRoom {
    ...
    constructor(options) {
        super(options);
        this.userid = this.id.slice(5);
        this.isSelf = (this.userid === PS.user.userid);
        this.name = options.username || this.userid;
        if (/[a-zA-Z0-9]/.test(this.name.charAt(0)))
            this.name = ' ' + this.name;
        PS.send(`|/cmd userdetails ${this.userid}`);
    }
}
class UserPanel extends PSRoomPanel {
    render() {
        ...
        if (globalGroup.name === group.name)
            groupName = null;
        let roomsList = null;
        if (user.rooms) {
            let battlebuf = [];
            ...
            for (let roomid in user.rooms) {
                ...
                if (roomid.substr(0, 7) === 'battle-') {
                    ...
                    const roomLink = <a href={`/${roomid}`} class={'ilink' + (ownBattle || roomid in PS.rooms ? ' yours' : '')} title={`${p1 || '?'} v. ${p2 || '?'}`}>{roomrank}{roomid.substr(7)}</a>;
            ...
            if (battlebuf.length)
                battlebuf.unshift(<br />, <em>Battles:</em>, " ");
            if (chatbuf.length)
                chatbuf.unshift(<br />, <em>Chatrooms:</em>, " ");
            if (privatebuf.length)
                privatebuf.unshift(<br />, <em>Private rooms:</em>, " ");
    ...
    render() {
        ...
        return <PSPanelWrapper room={room}>
			...
			<p class="volume">
				...
				{PS.prefs.mute ?
                <em>(muted)</em> :
                <input type="range" min="0" max="100" step="1" name="effectvolume" value={PS.prefs.effectvolume} onChange={this.setVolume} onInput={this.setVolume} onKeyUp={this.setVolume}/>}
			</p>
			<p class="volume">
				<label class="optlabel">Music: <span class="value">{!PS.prefs.mute && PS.prefs.musicvolume ? `${PS.prefs.musicvolume}%` : `muted`}</span></label>
				{PS.prefs.mute ?
                <em>(muted)</em> :
                <input type="range" min="0" max="100" step="1" name="musicvolume" value={PS.prefs.musicvolume} onChange={this.setVolume} onInput={this.setVolume} onKeyUp={this.setVolume}/>}
			</p>
			<p class="volume">
				...
				{PS.prefs.mute ?
                <em>(muted)</em> :
                <input type="range" min="0" max="100" step="1" name="notifvolume" value={PS.prefs.notifvolume} onChange={this.setVolume} onInput={this.setVolume} onKeyUp={this.setVolume}/>}
			</p>
			<p>
