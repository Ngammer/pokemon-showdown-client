...
class ChatRoom extends PSRoom {
    ...
    handleMessage(line) {
        ...
        switch (cmd) {
            case 'j':
            case 'join': {
                const roomid = /[^a-z0-9-]/.test(target) ? toID(target) : target;
                PS.join(roomid);
                return true;
            }
            case 'part':
    ...
    send(line, direct) {
        ...
        if (!direct && !line)
            return;
        if (!direct && this.handleMessage(line))
            return;
        if (this.pmTarget) {
...
class ChatTextEntry extends preact.Component {
    ...
    setValue(value, selection) {
        if (this.miniedit) {
            ...
            if (selection)
                this.textbox.setSelectionRange?.(selection.start, selection.end);
        }
    }
    historyUp() {
    ...
    render() {
        ...
        return <div class="chat-log-add hasuserlist" onClick={this.focusIfNoSelection} style={{ left: this.props.left || 0 }}>
			<form class="chatbox">
				<label style={{ color: BattleLog.usernameColor(PS.user.userid) }}>{PS.user.name}:</label>
				{OLD_TEXTBOX ? <textarea class={this.props.room.connected ? 'textbox' : 'textbox disabled'} autofocus rows={1} onInput={this.update} onKeyDown={this.onKeyDown} style={{ resize: 'none', width: '100%', height: '16px', padding: '2px 3px 1px 3px' }} placeholder={PS.focusPreview(this.props.room)}/> : <ChatTextBox class={this.props.room.connected ? 'textbox' : 'textbox disabled'} placeholder={PS.focusPreview(this.props.room)}/>}
			</form>
		</div>;
    }
}
class ChatTextBox extends preact.Component {
...
class ChatPanel extends PSRoomPanel {
    send = (text) => {
    ...
    focus() {
        ...
        setTimeout(() => {
            this.base.querySelector('textarea, pre.textbox').focus();
        }, 0);
    }
    focusIfNoSelection = () => {
        const selection = window.getSelection();
    ...
    makeChallenge = (e, format, team) => {
        const room = this.props.room;
        const packedTeam = team ? team.packedTeam : '';
        if (!room.pmTarget)
            throw new Error("Not a PM room");
...
class ChatUserList extends preact.Component {
    subscription = null;
    state = {
        expanded: false,
    };
    ...
    render() {
        const room = this.props.room;
        let userList = Object.entries(room.users);
        PSUtils.sortBy(userList, ([id, name]) => ([PS.server.getGroup(name.charAt(0)).order, !name.endsWith('@!'), id]));
...
class ChatLog extends preact.Component {
    ...
    shouldComponentUpdate(props) {
        ...
        if (props.left !== this.props.left)
            this.base.style.left = `${props.left || 0}px`;
        if (props.top !== this.props.top)
            this.base.style.top = `${props.top || 0}px`;
    ...
    setControlsJSX(jsx) {
        ...
        if (controlsElem && controlsElem.className !== 'controls')
