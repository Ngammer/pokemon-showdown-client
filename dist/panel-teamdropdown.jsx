...
class PSTeambuilder {
    ...
    static unpackTeam(buf) {
        ...
        for (const setBuf of buf.split(`]`)) {
            ...
            // moves
            set.moves = parts[4].split(',').map(moveid => Dex.moves.get(moveid).name);
            // nature
            set.nature = parts[5];
    ...
    static exportSet(set) {
        ...
        if (set.ivs) {
            for (const stat of Dex.statNames) {
                if (set.ivs[stat] === undefined || isNaN(set.ivs[stat]) || set.ivs[stat] === 31)
                    continue;
                if (first) {
                    text += `IVs: `;
                    first = false;
        ...
        for (let line of lines) {
            ...
            if (line === '' || line === '---') {
                ...
                if (!curTeam.format)
                    ...
                    curTeam.format = `gen6${curTeam.format}`;
    ...
    if (team) {
        ...
        if (!icons) {
            if (!team.packedTeam) {
                ...
                icons = PSTeambuilder.packedTeamNames(team.packedTeam).map(species => <span class="picon" style={Dex.getPokemonIcon(species)}></span>);
            }
            team.iconCache = icons;
        }
        let format = team.format;
        if (format.startsWith('gen8'))
...
class TeamDropdownPanel extends PSRoomPanel {
    ...
    format = null;
    getTeams() {
        if (!this.format && !this.gen)
            return PS.teams.list;
        return PS.teams.list.filter(team => {
            if (this.gen && !team.format.startsWith(this.gen))
                return false;
            if (this.format && team.format !== this.format)
                return false;
            return true;
        });
    }
    ...
    render() {
        ...
        let width = 307;
        if (availableWidth > 636)
            width = 613;
        if (availableWidth > 945)
            width = 919;
        let teamBuckets = {};
    ...
    render() {
        ...
        return <PSPanelWrapper room={room} width={width}>
			{columns.map(column => <ul class="options" onClick={this.click}>
				{column.map(format => format.id ? (<li><button value={format.name} class="option">
