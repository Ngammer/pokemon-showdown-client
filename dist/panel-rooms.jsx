...
class RoomsPanel extends PSRoomPanel {
    ...
    keyDownSearch = (e) => {
        ...
        if (e.keyCode === 13) {
            ...
            if (arrowIndex >= 0)
                value = value.slice(arrowIndex + 3);
    ...
    runSearch() {
        ...
        let abbr = roomList.filter(room => toID(room.title.toLowerCase().replace(/\b([a-z0-9])[a-z0-9]*\b/g, '$1')).startsWith(searchid) ||
        ...
        if (autoFill) {
            ...
            while (searchid !== toID(firstTitle.slice(0, firstTitleOffset)) &&
    ...
    render() {
        if (this.hidden && PS.isVisible(this.props.room))
            this.hidden = false;
        if (this.hidden) {
        ...
        return <PSPanelWrapper room={this.props.room} scrollable><div class="pad">
			...
			<div class="roomcounters">
				<button class="button" data-href="/users" title="Find an online user">
					<span class="pixelated usercount" title="Meloetta is PS's mascot! The Aria forme is about using its voice, and represents our chatrooms."></span>
					<strong>{rooms.userCount || '-'}</strong> users online
				</button> 
				<button class="button" data-href="/battles" title="Watch an active battle">
					<span class="pixelated battlecount" title="Meloetta is PS's mascot! The Pirouette forme is Fighting-type, and represents our battles."></span>
					<strong>{rooms.battleCount || '-'}</strong> active battles
				</button>
			</div>
			<div>
				<input type="search" name="roomsearch" class="textbox" style="width: 100%; max-width: 480px" placeholder="Join or search for rooms" onInput={this.changeSearch} onKeyDown={this.keyDownSearch}/>
			</div>
			{PS.isOffline ? <h2>(offline)</h2> : rooms.userCount === undefined && <h2>Connecting...</h2>}
			{roomList}
		</div></PSPanelWrapper>;
    }
    renderRoomList(title, rooms) {
