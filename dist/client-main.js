 ...
 */
class PSPrefs extends PSStreamModel {
    /**
    ...
    storageEngine = '';
    storage = {};
    origin = `https://${Config.routes.client}`;
    constructor() {
        super();
        for (const key in this) {
            const value = this[key];
            if (['storage', 'subscriptions', 'origin', 'storageEngine'].includes(key))
                continue;
 ...
 */
class PSTeams extends PSStreamModel {
    ...
    teambuilderFormat(format) {
        ...
        if (ruleSepIndex >= 0)
            format = format.slice(0, ruleSepIndex);
    ...
    packAll(teams) {
        return teams.map(team => ((team.format ? `${team.format}]` : ``) + (team.folder ? `${team.folder}/` : ``) + team.name + `|` + team.packedTeam)).join('\n');
    }
    save() {
        try {
    ...
    unpackLine(line) {
        ...
        if (slashIndex < 0)
            slashIndex = bracketIndex; // line.slice(slashIndex + 1, pipeIndex) will be ''
...
class PSServer {
    ...
    prefix = '/showdown';
    protocol = Config.defaultserver.httpport ? 'https' : 'http';
    groups = {
        '~': {
            name: "Administrator (~)",
            type: 'leadership',
...
class PSRoom extends PSStreamModel {
    ...
    constructor(options) {
        ...
        if (this.location !== 'popup' && this.location !== 'semimodal-popup')
            this.parentElem = null;
        if (options.rightPopup)
    ...
    send(msg, direct) {
        if (!direct && !msg)
            return;
        if (!direct && this.handleMessage(msg))
            return;
        PS.send(this.id + '|' + msg);
    }
    destroy() {
...
const PS = new class extends PSModel {
    ...
    update(layoutAlreadyUpdated) {
        if (!layoutAlreadyUpdated)
    ...
    send(fullMsg) {
        ...
        const roomid = fullMsg.slice(0, pipeIndex);
    ...
    updateRoomTypes() {
        ...
        for (const roomid in this.rooms) {
            ...
            if (this.activePanel === room)
                this.activePanel = newRoom;
            if (this.room === room)
                this.room = newRoom;
            if (roomid === '')
                this.mainmenu = newRoom;
            if (options.queue) {
    ...
    getPMRoom(userid) {
        ...
        const roomid = `pm-${[userid, myUserid].sort().join('-')}`;
        if (this.rooms[roomid])
            return this.rooms[roomid];
        this.join(roomid);
        return this.rooms[roomid];
    }
    addRoom(options, noFocus) {
    ...
    removeRoom(room) {
        ...
        if (PS.leftRoom === room) {
            ...
            if (PS.activePanel === room)
                PS.activePanel = this.mainmenu;
            if (PS.room === room)
                PS.room = this.mainmenu;
        }
        ...
        if (PS.rightRoom === room) {
            ...
            if (PS.activePanel === room)
                PS.activePanel = PS.rightRoom || PS.leftRoom;
            if (PS.room === room)
                PS.room = PS.activePanel;
        }
