...
const Dex = new class {
    ...
    resourcePrefix = (() => {
		...
		if (window.location.hostname === 'localhost' || window.location.hostname.startsWith('192.168.')) {
			 ...
			 // External
			 prefix = 'http://181.99.176.203:3000/';
		}
		return prefix;
  })();
    fxPrefix = (() => {
    ...
    types = {
        ...
        all: () => {
            if (this.types.allCache)
                return this.types.allCache;
            const types = [];
            for (const id in (window.BattleTypeChart || {})) {
                types.push(Dex.types.get(id));
            }
            if (types.length)
                this.types.allCache = types;
            return types;
        },
        isName: (name) => {
            const id = toID(name);
    ...
    getTeambuilderSpriteData(pokemon, gen = 0) {
    ...
    getPokeballs() {
        ...
        if (!window.BattleItems)
            window.BattleItems = {};
        for (const data of Object.values(window.BattleItems)) {
            if (!data.isPokeball)
                continue;
...
class ModdedDex {
    ...
    moves = {
        get: (name) => {
            ...
            if (this.cache.Moves.hasOwnProperty(id))
                return this.cache.Moves[id];
    ...
    items = {
        get: (name) => {
            ...
            if (this.cache.Items.hasOwnProperty(id))
    ...
    types = {
        get: (name) => {
            ...
            if (this.cache.Types.hasOwnProperty(id))
                return this.cache.Types[id];
...
const Teams = new class {
    ...
    export(team, gen, hidestats = false) {
        if (!team)
            return '';
