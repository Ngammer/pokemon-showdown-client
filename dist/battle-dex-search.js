...
class BattleTypedSearch {
    ...
    eggMovesOnly(child, father) {
        if (this.dex.species.get(child).baseSpecies === this.dex.species.get(father).baseSpecies)
            return false;
...
class BattlePokemonSearch extends BattleTypedSearch {
    ...
    getBaseResults() {
        ...
        if (format === '35pokes' && table.thirtyfivePokes) {
            tierSet = tierSet.filter(([type, id]) => {
                if (id in table.thirtyfivePokes)
                    return true;
                return false;
            });
        }
...
class BattleMoveSearch extends BattleTypedSearch {
    ...
    getBaseResults() {
        ...
        if (this.formatType === 'bw1')
            lsetTable = lsetTable['gen5bw1'];
        if (this.formatType?.startsWith('ssdlc1'))
            lsetTable = lsetTable['gen8dlc1'];
        if (this.formatType?.startsWith('predlc'))
            lsetTable = lsetTable['gen9predlc'];
        if (this.formatType?.startsWith('svdlc1'))
            lsetTable = lsetTable['gen9dlc1'];
        while (learnsetid) {
            ...
            if (learnset) {
                for (let moveid in learnset) {
                    ...
                    if (this.eggMovesOnly(learnsetid, species.id) &&
                    ...
                    if (this.formatType?.includes('predlc') && this.formatType !== 'predlcnatdex' &&
                        BattleTeambuilderTable['gen9predlc']?.nonstandardMoves.includes(moveid)) {
                        continue;
                    }
                    if (this.formatType?.includes('svdlc1') && this.formatType !== 'svdlc1natdex' &&
        ...
        if (sketch || isHackmons) {
            ...
            for (let id in BattleMovedex) {
                ...
                if (move.gen > dex.gen)
                    continue;
                if (sketch) {
                    if (move.flags['nosketch'] || move.isMax || move.isZ)
                        continue;
            ...
            for (let id in this.getTable()) {
                ...
                for (let i = dex.gen; i >= species.gen && i >= move.gen; i--) {
                    ...
                    const excludedForme = (s) => [
                        'Alola', 'Alola-Totem', 'Galar', 'Galar-Zen', 'Hisui', 'Hisui-Totem', 'Paldea', 'Paldea-Combat', 'Paldea-Blaze', 'Paldea-Aqua', 'Totem-Serious', 'Totem-Wrath', 'Totem-Grief',
                    ].includes(s.forme);
