...
class ModifiableValue {
    ...
    weatherName;
    isAccuracy = false;
    constructor(battle, pokemon, serverPokemon) {
        this.comment = [];
        this.battle = battle;
    ...
    tryAbility(abilityName) {
        ...
        if (!this.pokemon?.effectiveAbility(this.serverPokemon))
            return false;
        return true;
    }
    tryWeather(weatherName) {
        if (!this.weatherName)
            return false;
        if (!weatherName)
            weatherName = this.weatherName;
        else if (weatherName !== this.weatherName)
            return false;
    ...
    itemModify(factor, itemName) {
        if (!itemName)
            itemName = this.itemName;
        if (!itemName)
            return false;
        if (!this.tryItem(itemName))
            return false;
        return this.modify(factor, itemName);
    }
    abilityModify(factor, abilityName) {
        if (!this.tryAbility(abilityName))
            return false;
        return this.modify(factor, abilityName);
    }
    weatherModify(factor, weatherName, name) {
        if (!weatherName)
            weatherName = this.weatherName;
        if (!weatherName)
            return false;
        if (!this.tryWeather(weatherName))
            return false;
    ...
    modify(factor, name) {
        ...
        if (this.battle.tier.includes('Super Staff Bros') &&
            !(name === 'Confirmed Town' && this.maxValue > 60))
            this.maxValue *= factor;
        return true;
    }
    set(value, reason) {
        if (reason)
            this.comment.push(` (${reason})`);
        this.value = value;
        this.maxValue = 0;
        return true;
    }
    setRange(value, maxValue, reason) {
        if (reason)
            this.comment.push(` (${reason})`);
        this.value = value;
...
class BattleTooltips {
    ...
    placeTooltip(innerHTML, hoveredElem, notRelativeToParent, type) {
        let $elem;
        if (hoveredElem) {
            $elem = $(hoveredElem);
        }
    ...
    getStatusZMoveEffect(move) {
        if (move.zMove.effect in BattleTooltips.zMoveEffects) {
            return BattleTooltips.zMoveEffects[move.zMove.effect];
        }
        let boostText = '';
        if (move.zMove.boost) {
            let boosts = Object.keys(move.zMove.boost);
            boostText = boosts.map(stat => BattleTextParser.stat(stat) + ' +' + move.zMove.boost[stat]).join(', ');
        }
        return boostText;
    }
    static zMoveTable = {
    ...
    getMaxMoveFromType(type, gmaxMove) {
        if (gmaxMove) {
            if (typeof gmaxMove === 'string')
                gmaxMove = this.battle.dex.moves.get(gmaxMove);
            if (type === gmaxMove.type)
                return gmaxMove;
        }
        return this.battle.dex.moves.get(BattleTooltips.maxMoveTable[type]);
    }
    showMoveTooltip(move, isZOrMax, pokemon, serverPokemon, gmaxMove) {
        let text = '';
        let zEffect = '';
        ...
        if (category !== 'Status' && foeActive.length > 1) {
            ...
            for (const active of foeActive) {
                ...
                basePower = '' + value;
                if (prevBasePower === null)
                    prevBasePower = basePower;
                if (prevBasePower !== basePower)
                    difference = true;
    ...
    showPokemonTooltip(clientPokemon, serverPokemon, isActive, illusionIndex) {
        ...
        if (supportsAbilities) {
            abilityText = this.getPokemonAbilityText(clientPokemon, serverPokemon, isActive, !!illusionIndex && illusionIndex > 1);
        }
        let itemText = '';
        if (serverPokemon) {
            let item = '';
            ...
            if (serverPokemon.item)
                item = this.battle.dex.items.get(serverPokemon.item).name;
            if (itemEffect)
                itemEffect = ' (' + itemEffect + ')';
            if (item)
                itemText = '<small>Item:</small> ' + item + itemEffect;
        }
            ...
            if (pokemon.item)
                item = this.battle.dex.items.get(pokemon.item).name;
            if (itemEffect)
                itemEffect = ' (' + itemEffect + ')';
            if (item)
                itemText = '<small>Item:</small> ' + item + itemEffect;
     ...
     * and shininess.)
     */
    pokemonHasClones(pokemon) {
        const side = pokemon.side;
        if (side.battle.speciesClause)
            return false;
    ...
    calculateModifiedStats(clientPokemon, serverPokemon, statStagesOnly) {
        ...
        if ((ability === 'klutz' && !speedHalvingEVItems.includes(item)) ||
        ...
        if (clientPokemon && (ability === 'plus' || ability === 'minus')) {
            ...
            if (allyActive.length > 1) {
                ...
                for (const ally of allyActive) {
                    if (!ally || ally === clientPokemon || ally.fainted)
                        continue;
                    let allyAbility = this.getAllyAbility(ally);
                    if (allyAbility !== 'Plus' && allyAbility !== 'Minus')
                        continue;
                    if (this.battle.gen <= 4 && allyAbility === abilityName)
                        continue;
                    stats.spa = Math.floor(stats.spa * 1.5);
                    break;
                }
    ...
    renderStats(clientPokemon, serverPokemon, short) {
        ...
        if (!short) {
            ...
            for (const statName of Dex.statNamesExceptHP) {
                if (this.battle.gen === 1 && statName === 'spd')
                    continue;
    ...
    getPPUseText(moveTrackRow, showKnown) {
    ...
    getSpeedRange(pokemon) {
        ...
        if (tier.includes("Let's Go")) {
            ...
            if (tier.includes('No Restrictions'))
                max += 200;
            else if (tier.includes('Random'))
                max += 20;
        }
        else {
     ...
     */
    getMoveType(move, value, forMaxMove) {
        const pokemon = value.pokemon;
        const serverPokemon = value.serverPokemon;
        let moveType = move.type;
        let category = move.category;
        if (category === 'Status' && forMaxMove)
            return ['Normal', 'Status']; // Max Guard
        ...
        if (allowTypeOverride) {
            ...
            if (category !== 'Status' && !move.isZ && !move.id.startsWith('hiddenpower')) {
                if (moveType === 'Normal') {
                    if (value.abilityModify(0, 'Aerilate'))
                        moveType = 'Flying';
            ...
            const isSound = !!(forMaxMove ?
                this.getMaxMoveFromType(moveType, forMaxMove !== true && forMaxMove || undefined) : move).flags['sound'];
        ...
        if (this.battle.tier.includes('Super Staff Bros')) {
            if (allowTypeOverride && category !== "Status" && !move.isZ && !move.id.startsWith('hiddenpower')) {
                if (value.abilityModify(0, 'Acetosa'))
                    moveType = 'Grass';
                if (value.abilityModify(0, 'I Can Hear The Heart Beating As One') && moveType === 'Normal')
                    moveType = 'Fairy';
            }
            ...
            if (move.id === 'weatherball' && value.weatherModify(0)) {
                if (this.battle.weather === 'stormsurge')
                    moveType = 'Water';
                if (this.battle.weather === 'deserteddunes')
                    moveType = 'Rock';
            }
            ...
            if (move.id === 'asongoficeandfire' && pokemon.getSpeciesForme() === 'Volcarona')
                moveType = 'Ice';
    ...
    getMoveAccuracy(move, value, target) {
        ...
        if (move.id === 'hurricane' || move.id === 'thunder') {
            if (value.tryWeather('Sunny Day'))
                value.set(50, 'Sunny Day');
    ...
    getMoveBasePower(move, moveType, value, target = null) {
        const pokemon = value.pokemon;
        const serverPokemon = value.serverPokemon;
        ...
        if (move.flags['slicing']) {
		  ...
		  if (move.flags['bullet']) {
			value.abilityModify(1.5, "Bullet");
			}
		  if (move.type === 'Bug') {
        ...
        if (pokemon.terastallized && (pokemon.terastallized === move.type || pokemon.terastallized === 'Stellar') &&
        ...
        if (this.battle.tier.includes('Super Staff Bros')) {
            if (move.id === 'bodycount') {
                value.set(50 + 50 * pokemon.side.faintCounter, pokemon.side.faintCounter > 0
                    ? `${pokemon.side.faintCounter} teammate${pokemon.side.faintCounter > 1 ? 's' : ''} KOed`
                    : undefined);
            }
            // Base power based on times hit
            if (move.id === 'vengefulmood') {
                value.set(Math.min(140, 60 + 20 * pokemon.timesAttacked), pokemon.timesAttacked > 0
                    ? `Hit ${pokemon.timesAttacked} time${pokemon.timesAttacked > 1 ? 's' : ''}`
                    : undefined);
            }
            if (move.id === 'alting' && pokemon.shiny) {
            ...
            if (move.category !== 'Status' && allowTypeOverride && !move.isZ &&
                ...
                if (moveType === 'Normal')
                    value.abilityModify(this.battle.gen > 6 ? 1.2 : 1.3, "I Can Hear The Heart Beating As One");
    ...
    getItemBoost(move, value, moveType) {
        ...
        // Gems
        if (BattleTooltips.noGemMoves.includes(moveName))
            return value;
    ...
    getPokemonAbilityData(clientPokemon, serverPokemon) {
        const abilityData = {
            ...
            if (clientPokemon.ability) {
                ...
                if (species.exists && species.abilities) {
                    ...
                    if (this.battle.rules['Frantic Fusions Mod']) {
                        ...
                        if (fusionSpecies.exists && fusionSpecies.name !== species.name) {
                            abilityData.possibilities = Array.from(new Set(abilityData.possibilities.concat(Object.values(fusionSpecies.abilities))));
                        }
                    }
                }
            }
    ...
    getPokemonAbilityText(clientPokemon, serverPokemon, isActive, hidePossible) {
        let text = '';
        ...
        if (!isActive) {
            ...
            if (ability)
                text = '<small>Ability:</small> ' + this.battle.dex.abilities.get(ability).name;
        }
        else {
            if (abilityData.ability) {
...
class BattleStatGuesser {
    ...
    constructor(formatid) {
        ...
        this.ignoreEVLimits = (this.dex.gen < 3 ||
    ...
    guessRole(set) {
        ...
        if (moveids.includes('lastresort'))
            needsFourMoves = false;
        ...
        for (let i = 0, len = set.moves.length; i < len; i++) {
            ...
            if (move.category === 'Status') {
                ...
                else if (['protect', 'detect', 'spikyshield', 'kingsshield', 'shelter', 'watersport'].includes(move.id)) {
                    moveCount['Stall']++;
                }
        ...
        if (hasMove['agility'] || hasMove['dragondance'] || hasMove['quiverdance'] ||
    ...
    ensureMinEVs(evs, stat, min, evTotal) {
        if (!evs[stat])
            evs[stat] = 0;
    ...
    ensureMaxEVs(evs, stat, min, evTotal) {
        if (!evs[stat])
            evs[stat] = 0;
    ...
    guessEVs(set, role) {
        ...
        if (this.supportsAVs) {
            ...
            if (!moveCount['PhysicalAttack'])
                evs.atk = 0;
            if (!moveCount['SpecialAttack'] && this.dex.gen > 1)
                evs.spa = 0;
            if (hasMove['gyroball'] || hasMove['trickroom'])
                evs.spe = 0;
            if (this.dex.gen === 1)
            ...
            let ev = 252;
            while (ev > 0 && stat <= this.getStat(primaryStat, set, ev - 4, plusStat === primaryStat ? 1.1 : 1.0))
                ev -= 4;
            evs[primaryStat] = ev;
            evTotal += ev;
            let secondaryStat = statChart[role][1];
            if (secondaryStat === 'hp' && set.level && set.level < 20)
                secondaryStat = 'spd';
            stat = this.getStat(secondaryStat, set, 252, plusStat === secondaryStat ? 1.1 : 1.0);
            ev = 252;
            while (ev > 0 && stat <= this.getStat(secondaryStat, set, ev - 4, plusStat === secondaryStat ? 1.1 : 1.0))
                ev -= 4;
            ...
            if (evTotal < 508) {
                ...
                if (secondaryStat) {
                    ...
                    while (ev > 0 && stat === this.getStat(secondaryStat, set, ev - 4))
                        ev -= 4;
                    if (ev)
                        evs[secondaryStat] = ev;
                    remaining -= ev;
    ...
    getStat(stat, set, evOverride, natureOverride) {
        ...
        if (typeof ev !== 'number')
            ev = (this.dex.gen > 2 ? 0 : 252);
        if (evOverride !== undefined)
            ev = evOverride;
        if (stat === 'hp') {
            if (baseStat === 1)
                return 1;
            if (!this.supportsEVs)
                return ~~(~~(2 * baseStat + iv + 100) * level / 100 + 10) + (this.supportsAVs ? ev : 0);
...
function BattleStatOptimizer(set, formatid) {
    if (!set.evs)
        return null;
    const dex = Dex.mod(formatid.slice(0, 4));
    const ignoreEVLimits = (dex.gen < 3 ||
    ...
    if (!supportsEVs || ignoreEVLimits)
        return false;
    const species = dex.species.get(set.species);
    const level = set.level || 100;
    const getStat = (stat, ev, nature) => {
    ...
    if (bestPlus && savedEVs >= 0) {
        ...
        if (bestPlus !== origNature.plus || bestMinus !== origNature.minus) {
            ...
            for (const stat of Dex.statNames) {
                if (!newSpread.evs[stat])
                    delete newSpread.evs[stat];
            }
