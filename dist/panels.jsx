...
class PSRoomPanel extends preact.Component {
    subscriptions = [];
    componentDidMount() {
        if (PS.room === this.props.room)
            this.focus();
...
function PSPanelWrapper(props) {
    ...
    return <div class={'ps-room' + (room.id === '' ? '' : ' ps-room-light') + (props.scrollable ? ' scrollable' : '')} id={`room-${room.id}`} style={style}>
		{props.children}
	</div>;
}
class PSMain extends preact.Component {
    constructor() {
        ...
        window.addEventListener('click', e => {
            ...
            while (elem) {
                ...
                if (elem.tagName === 'A' || elem.getAttribute('data-href')) {
                    const href = elem.getAttribute('data-href') || elem.href;
        ...
        if (colorSchemeQuery?.media !== 'not all') {
            colorSchemeQuery.addEventListener('change', function (cs) {
                if (PS.prefs.theme === 'system')
                    document.body.className = cs.matches ? 'dark' : '';
            });
        }
    ...
    static posStyle(room) {
        ...
        if (PS.leftRoomWidth === 0) {
            // one panel visible
            if (room === PS.activePanel)
                pos = { top: 56 };
        }
        else {
            // both panels visible
            if (room === PS.leftRoom)
                pos = { top: 56, right: PS.leftRoomWidth };
            if (room === PS.rightRoom)
                pos = { top: 56, left: PS.leftRoomWidth };
        }
        if (!pos)
            return { display: 'none' };
        let top = (pos.top || 0);
        let height = null;
        let bottom = (pos.bottom || 0);
        if (bottom > 0 || top < 0) {
            height = bottom - top;
            if (height < 0)
                throw new RangeError("Invalid pos range");
            if (top < 0)
                top = null;
            else
                bottom = null;
    ...
    static getPopupStyle(room, width) {
