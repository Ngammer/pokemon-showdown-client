...
class MainMenuRoom extends PSRoom {
    ...
    handlePM(user1, user2, message) {
        ...
        const roomid = `pm-${[userid1, userid2].sort().join('-')}`;
    ...
    handleQueryResponse(id, response) {
        switch (id) {
            case 'userdetails':
                ...
                const userRoom = PS.rooms[`user-${userid}`];
                if (userRoom)
                    userRoom.update(null);
                break;
            case 'rooms':
                if (response.pspl) {
                ...
                const roomsRoom = PS.rooms[`rooms`];
                if (roomsRoom)
                    roomsRoom.update(null);
                break;
            case 'roomlist':
                const battlesRoom = PS.rooms[`battles`];
                if (battlesRoom) {
...
class FormatDropdown extends preact.Component {
    ...
    render() {
        if (this.props.format) {
            return <button name="format" value={this.props.format} class="select formatselect preselected" disabled>{this.props.format}</button>;
        }
        return <button name="format" value={this.format} class="select formatselect" data-href="/formatdropdown" onChange={this.change}>
			{this.format}
		</button>;
    }
}
class TeamDropdown extends preact.Component {
    teamFormat = '';
    teamKey = '';
    change = () => {
        if (!this.base)
    ...
    getDefaultTeam(teambuilderFormat) {
        for (const team of PS.teams.list) {
            if (team.format === teambuilderFormat)
                return team.key;
        }
        return '';
    }
    render() {
        ...
        return <button name="team" value={this.teamKey} class="select teamselect" data-href="/teamdropdown" data-format={teamFormat} onChange={this.change}>
...
class TeamForm extends preact.Component {
    ...
    submit = (e) => {
        e.preventDefault();
        const format = this.base.querySelector('button[name=format]').value;
