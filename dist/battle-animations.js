...
exports.BattleStatusAnims = exports.BattleOtherAnims = exports.PokemonSprite = exports.Sprite = exports.BattleScene = void 0;
const battle_animations_moves_1 = require("./battle-animations-moves");
const battle_log_1 = require("./battle-log");
const battle_sound_1 = require("./battle-sound");
/*
...
class BattleScene {
    ...
    constructor(battle, $frame, $logFrame) {
        ...
        this.$frame = $frame;
        this.log = new battle_log_1.BattleLog($logFrame[0], this);
        this.log.battleParser.pokemonName = (pokemonId) => {
            if (!pokemonId)
                return '';
            ...
            if (pokemonId.charAt(3) === ':')
                return BattleTextParser.escapeReplace(pokemonId.slice(4).trim());
            else if (pokemonId.charAt(2) === ':')
                return BattleTextParser.escapeReplace(pokemonId.slice(3).trim());
            return '???pokemon:' + pokemonId + '???';
        };
        let numericId = 0;
        if (battle.id) {
            numericId = parseInt(battle.id.slice(battle.id.lastIndexOf('-') + 1), 10);
            if (this.battle.id.includes('digimon'))
                this.mod = 'digimon';
        }
        if (!numericId) {
    ...
    setMute(muted) {
        battle_sound_1.BattleSound.setMute(muted);
    }
    wait(time) {
        if (!this.animating)
            return;
    ...
    addSprite(sprite) {
        if (sprite.$el)
            this.$sprites[+sprite.isFrontSprite].append(sprite.$el);
    }
    showEffect(effect, start, end, transition, after, additionalCss) {
        if (typeof effect === 'string')
            effect = BattleEffects[effect];
        if (!start.time)
            start.time = 0;
        if (!end.time)
        ...
        if (!end.scale && end.scale !== 0 && start.scale)
    ...
    pos(loc, obj) {
        ...
        if (!loc.xscale && loc.xscale !== 0)
            loc.xscale = loc.scale;
        if (!loc.yscale && loc.yscale !== 0)
            loc.yscale = loc.scale;
        let left = 210;
        let top = 245;
        let scale = (obj.gen === 5
            ? 2.0 - ((loc.z) / 200)
     ...
     */
    posT(loc, obj, transition, oldLoc) {
    ...
    runMoveAnim(moveid, participants) {
        if (!this.animating)
            return;
        let animEntry = battle_animations_moves_1.BattleMoveAnims[moveid];
        if (this.acceleration >= 3) {
            ...
            animEntry = exports.BattleOtherAnims[targetsSelf ? 'fastanimself' : isSpecial ? 'fastanimspecial' : 'fastanimattack'];
        }
        else if (!animEntry) {
            animEntry = battle_animations_moves_1.BattleMoveAnims['tackle'];
        }
    ...
    runOtherAnim(moveid, participants) {
        if (!this.animating)
            return;
        exports.BattleOtherAnims[moveid].anim(this, participants.map(p => p.sprite));
    }
    runStatusAnim(moveid, participants) {
        if (!this.animating)
            return;
        exports.BattleStatusAnims[moveid].anim(this, participants.map(p => p.sprite));
    }
    runResidualAnim(moveid, pokemon) {
        if (!this.animating)
            return;
        battle_animations_moves_1.BattleMoveAnims[moveid].residualAnim(this, [pokemon.sprite]);
    }
    runPrepareAnim(moveid, attacker, defender) {
        if (!this.animating || this.acceleration >= 3)
            return;
        const moveAnim = battle_animations_moves_1.BattleMoveAnims[moveid];
        if (!moveAnim.prepareAnim)
            return;
    ...
    updateGen() {
        ...
        if (isSPL) {
            if (gen <= 1)
                bg = 'fx/bg-gen1-spl.png';
            else if (gen <= 2)
                bg = 'fx/bg-gen2-spl.png';
            else if (gen <= 3)
                bg = 'fx/bg-gen3-spl.png';
            else if (gen <= 4)
                bg = 'fx/bg-gen4-spl.png';
            else
                bg = 'fx/bg-spl.png';
            this.setBgm(-101);
        }
    ...
    getDetailsText(pokemon) {
        ...
        return battle_log_1.BattleLog.escapeHTML(name);
    }
    getSidebarHTML(side, posStr) {
        ...
        const ratinghtml = side.rating ? ` title="Rating: ${battle_log_1.BattleLog.escapeHTML(side.rating)}"` : ``;
        ...
        return (`<div class="trainer trainer-${posStr}"${faded}><strong>${battle_log_1.BattleLog.escapeHTML(side.name)}</strong>` +
    ...
    teamPreview() {
        ...
        for (let siden = 0; siden < 2 || (this.battle.gameType === 'multi' && siden < 4); siden++) {
            ...
            for (let i = 0; i < side.pokemon.length; i++) {
                ...
                if (pokemon.speciesForme === 'Ludicolo')
                    ludicoloCount++;
                if (pokemon.speciesForme === 'Lombre')
                    lombreCount++;
            ...
            if (textBuf) {
                this.log.addDiv('chat battle-history', '<strong>' + battle_log_1.BattleLog.escapeHTML(side.name) + '\'s team:</strong> <em style="color:#445566;display:block;">' + battle_log_1.BattleLog.escapeHTML(textBuf) + '</em>');
            }
    ...
    showJoinButtons() {
        if (!this.battle.joinButtons)
            return;
        if (this.battle.ended || this.battle.rated)
            return;
    ...
    sideConditionLeft(cond, isFoe, all) {
        if (!cond[2] && !cond[3] && !all)
            return '';
    ...
    updateWeather(instant) {
        ...
        if (instant) {
            ...
            if (this.curWeather === weather && this.curTerrain === terrain)
                return;
    ...
    updateAcceleration() {
        if (this.battle.turnsSinceMoved > 2) {
            ...
            if (this.battle.messageFadeTime < 50)
                this.acceleration = 3;
        }
    }
    ...
    removeSideCondition(siden, id) {
        ...
        if (this.sideConditions[siden][id]) {
            for (const sprite of this.sideConditions[siden][id])
                sprite.destroy();
    ...
    resetSideConditions() {
        for (let siden = 0; siden < this.sideConditions.length; siden++) {
            ...
            for (const id in this.battle.sides[siden].sideConditions) {
                this.addSideCondition(siden, id, true);
            }
        }
    }
    typeAnim(pokemon, types) {
        const result = battle_log_1.BattleLog.escapeHTML(types).split('/').map(type => '<img src="' + Dex.resourcePrefix + 'sprites/types/' + encodeURIComponent(type) + '.png" alt="' + type + '" class="pixelated" />').join(' ');
    ...
    setBgm(bgmNum) {
        if (this.bgmNum === bgmNum)
            return;
        this.bgmNum = bgmNum;
        switch (bgmNum) {
            case -1:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/bw2-homika-dogars.mp3', 1661, 68131, this.bgm);
                break;
            case -2:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/xd-miror-b.mp3', 9000, 57815, this.bgm);
                break;
            case -3:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/colosseum-miror-b.mp3', 896, 47462, this.bgm);
                break;
            case 1:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/dpp-trainer.mp3', 13440, 96959, this.bgm);
                break;
            case 2:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/dpp-rival.mp3', 13888, 66352, this.bgm);
                break;
            case 3:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/hgss-johto-trainer.mp3', 23731, 125086, this.bgm);
                break;
            case 4:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/hgss-kanto-trainer.mp3', 13003, 94656, this.bgm);
                break;
            case 5:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/bw-trainer.mp3', 14629, 110109, this.bgm);
                break;
            case 6:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/bw-rival.mp3', 19180, 57373, this.bgm);
                break;
            case 7:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/bw-subway-trainer.mp3', 15503, 110984, this.bgm);
                break;
            case 8:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/bw2-kanto-gym-leader.mp3', 14626, 58986, this.bgm);
                break;
            case 9:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/bw2-rival.mp3', 7152, 68708, this.bgm);
                break;
            case 10:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/xy-trainer.mp3', 7802, 82469, this.bgm);
                break;
            case 11:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/xy-rival.mp3', 7802, 58634, this.bgm);
                break;
            case 12:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/oras-trainer.mp3', 13579, 91548, this.bgm);
                break;
            case 13:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/oras-rival.mp3', 14303, 69149, this.bgm);
                break;
            case 14:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/sm-trainer.mp3', 8323, 89230, this.bgm);
                break;
            case -101:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/spl-elite4.mp3', 3962, 152509, this.bgm);
                break;
            case 15:
            default:
                this.bgm = battle_sound_1.BattleSound.loadBgm('audio/sm-rival.mp3', 11389, 62158, this.bgm);
                break;
        }
        this.updateBgm();
    }
...
class Sprite {
    ...
    y;
    z;
    constructor(spriteData, pos, scene) {
        this.scene = scene;
        let sp = null;
        if (spriteData) {
            sp = spriteData;
...
class PokemonSprite extends Sprite {
    ...
    top = 0;
    effects = {};
    constructor(spriteData, pos, scene, isFrontSprite) {
    ...
    animSubFade(instant) {
        if (!this.$sub || !this.scene.animating)
            return;
    ...
    reset(pokemon) {
        ...
        for (const id in pokemon.turnstatuses)
            this.addEffect(id, true);
        for (const id in pokemon.movestatuses)
            this.addEffect(id, true);
    }
    animReset() {
        if (!this.scene.animating)
    ...
    recalculatePos(slot) {
        ...
        if (this.scene.gen <= 4 && moreActive) {
            ...
            if (this.isFrontSprite)
                statbarOffset = 30 * slot;
            if (!this.isFrontSprite)
                statbarOffset = -28 * slot;
        }
        else {
            switch (moreActive) {
                case 0:
    ...
    animSummon(pokemon, slot, instant) {
        ...
        if (this.cryurl) {
            battle_sound_1.BattleSound.playEffect(this.cryurl);
        }
        this.$el.css(this.scene.pos({
            display: 'block',
            x: this.x,
        ...
        if (this.sp.shiny && this.scene.acceleration < 2)
            exports.BattleOtherAnims.shiny.anim(this.scene, [this]);
        this.scene.waitFor(this.$el);
        if (pokemon.hasVolatile('substitute')) {
    ...
    animDragOut(pokemon) {
        if (!this.scene.animating)
            return this.animUnsummon(pokemon, true);
        if (this.$sub) {
            this.isSubActive = false;
            const $sub = this.$sub;
     ...
     */
    animTransform(pokemon, useSpeciesAnim, isPermanent) {
        if (!this.scene.animating && !isPermanent)
            return;
        ...
        if (useSpeciesAnim) {
            if (speciesid === 'kyogreprimal') {
                exports.BattleOtherAnims.primalalpha.anim(scene, [this]);
                doCry = true;
            }
            else if (speciesid === 'groudonprimal') {
                exports.BattleOtherAnims.primalomega.anim(scene, [this]);
                doCry = true;
            }
            else if (speciesid === 'necrozmaultra') {
                exports.BattleOtherAnims.ultraburst.anim(scene, [this]);
                doCry = true;
            }
            else if (speciesid === 'zygardecomplete') {
                exports.BattleOtherAnims.powerconstruct.anim(scene, [this]);
            }
            else if (speciesid === 'wishiwashischool' || speciesid === 'greninjaash') {
                exports.BattleOtherAnims.schoolingin.anim(scene, [this]);
            }
            else if (speciesid === 'wishiwashi') {
                exports.BattleOtherAnims.schoolingout.anim(scene, [this]);
            }
                ...
                exports.BattleOtherAnims.megaevo.anim(scene, [this]);
                doCry = true;
            }
        }
    ...
    removeEffect(id, instant) {
        if (id === 'formechange')
            this.removeTransform();
        if (id === 'substitute')
            this.animSubFade(instant);
        if (this.effects[id]) {
            for (const sprite of this.effects[id])
                sprite.destroy();
    ...
    getStatbarHTML(pokemon) {
        ...
        buf += `<strong>${battle_log_1.BattleLog.escapeHTML(ignoreNick ? pokemon.speciesForme : pokemon.name)}`;
        const gender = pokemon.gender;
        if (gender === 'M' || gender === 'F') {
    ...
    updateStatbar(pokemon, updatePrevhp, updateHp) {
        ...
        if (updatePrevhp) {
            ...
            if (hpcolor === 'g')
                $prevhp.removeClass('prevhp-yellow prevhp-red');
            else if (hpcolor === 'y')
    ...
    static getEffectTag(id) {
        let effect = PokemonSprite.statusTable[id];
        if (typeof effect === 'string')
            return effect;
        if (effect === null)
            return PokemonSprite.statusTable[id] = '';
        if (effect === undefined) {
            let label = `[[${id}]]`;
...
exports.PokemonSprite = PokemonSprite;
...
Object.assign($.easing, {
    ballisticUp(x, t, b, c, d) {
        return -3 * x * x + 4 * x;
    },
    ballisticDown(x, t, b, c, d) {
        x = 1 - x;
        return 1 - (-3 * x * x + 4 * x);
    },
    quadUp(x, t, b, c, d) {
...
(() => {
    if (!window.Dex || !Dex.resourcePrefix)
        return;
    for (const id in BattleEffects) {
        if (!BattleEffects[id].url)
            continue;
...
exports.BattleStatusAnims['focuspunch'] = { anim: exports.BattleStatusAnims['flinch'].anim };
