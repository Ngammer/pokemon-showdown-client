...
class PSSearchResults extends preact.Component {
    ...
    renderPokemonRow(id, matchStart, matchEnd, errorMessage) {
        ...
        if (!pokemon)
            return <li class="result">Unrecognized pokemon</li>;
        ...
        return <li class="result"><a href={`${this.URL_ROOT}pokemon/${id}`} data-target="push" data-entry={`pokemon|${pokemon.name}`}>
			...
			<span class="col typecol">
				{pokemon.types.map(type => <img src={`${Dex.resourcePrefix}sprites/types/${type}.png`} alt={type} height="14" width="32" class="pixelated"/>)}
			</span>
    ...
    renderName(name, matchStart, matchEnd, tagStart) {
        ...
        if (matchEnd && matchEnd > tagStart) {
            ...
            output.push(<small>{name.slice(tagStart, matchStart)}<b>{name.slice(matchStart, matchEnd)}</b>{name.slice(matchEnd)}</small>);
        }
        else {
    ...
    renderItemRow(id, matchStart, matchEnd, errorMessage) {
        ...
        if (!item)
            return <li class="result">Unrecognized item</li>;
    ...
    renderAbilityRow(id, matchStart, matchEnd, errorMessage) {
        ...
        if (!ability)
            return <li class="result">Unrecognized ability</li>;
    ...
    renderMoveRow(id, matchStart, matchEnd, errorMessage) {
        ...
        if (!move)
            return <li class="result">Unrecognized move</li>;
    ...
    renderRow(row) {
        ...
        else if ((label = search.illegalLabel(id))) { // tslint:disable-line
    ...
    render() {
        ...
        return <ul class="dexlist">
			{search.filters && <p>
				Filters: 
				{search.filters.map(([type, name]) => <button class="filter" value={`${type}:${name}`}>
